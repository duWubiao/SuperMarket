...
$(function(){
	...
	birthday.next().html("*");
	userRole.next().html("*");
	if(errorfile.val() == null || errorfile.val() == ""){
		a_idPicPath.next().html("*文件上传的大小必须小于500K,以jpg、jpeg、png、pneg为后缀");
	}else{
		a_idPicPath.next().html(errorfile);
	}
	
	if(errorWpfile.val() == null || errorWpfile.val() == ""){
		a_workPicPath.next().html("*文件上传的大小必须小于500K,以jpg、jpeg、png、pneg为后缀");
	}else{
		a_workPicPath.next().html(errorWpfile);
	}
	/*$.ajax({
		type:"GET",//请求类型
